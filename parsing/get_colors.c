...
/*   Updated: 2022/09/15 17:32:37 by aessakhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	tex->F++;
	if (tex->NO != 1 || tex->SO != 1 || tex->WE != 1 || tex->EA != 1 || tex->F != 1 || tex->C != 0 || tex->map != 0)
	{
		printf("Error\n");
		if (tex->F != 1)
			printf("Multiple RGB range for floor\n");
		if (tex->NO != 1 || tex->SO != 1 || tex->WE != 1 || tex->EA != 1 || tex->C != 0)
			printf("Wrong order\n");
		if (tex->map)
...
{
	tex->C++;
	if (tex->NO != 1 || tex->SO != 1 || tex->WE != 1 || tex->EA != 1 || tex->F != 1 || tex->C != 1 || tex->map != 0)
	{
		printf("Error\n");
		if (tex->C != 1)
			printf("Multiple RGB range for ceiling\n");
		if (tex->NO != 1 || tex->SO != 1 || tex->WE != 1 || tex->EA != 1 || tex->F != 1)
			printf("Wrong order\n");
		if (tex->map)
			printf("Wrong order: map before texture path");
		free_dbl_array(tmp);
