...
/*   Updated: 2022/10/03 19:51:46 by aessakhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	tex->NO++;
	if (tex->NO != 1 || tex->SO != 0 || tex->WE != 0
		|| tex->EA != 0 || tex->F != 0 || tex->C != 0 || tex->map != 0)
	{
		printf("Error\n");
		if (tex->NO != 1)
			printf("Multiple paths to north texture\n");
		if (tex->SO != 0 || tex->WE != 0
			|| tex->EA != 0 || tex->F != 0 || tex->C != 0)
			printf("Wrong order\n");
		if (tex->map)
...
{
	tex->SO++;
	if (tex->NO != 1 || tex->SO != 1 || tex->WE != 0
		|| tex->EA != 0 || tex->F != 0 || tex->C != 0 || tex->map != 0)
	{
		printf("Error\n");
		if (tex->SO != 1)
			printf("Multiple paths to south texture\n");
		if (tex->NO != 1 || tex->WE != 0
			|| tex->EA != 0 || tex->F != 0 || tex->C != 0)
			printf("Wrong order\n");
		if (tex->map)
			printf("Wrong order: map before texture path");
...
void	get_ea(t_texture_check *tex, char **tmp, t_map *map, int i)
{
	tex->EA++;
	if (tex->NO != 1 || tex->SO != 1 || tex->WE != 1
		|| tex->EA != 1 || tex->F != 0 || tex->C != 0 || tex->map != 0)
	{
		if (tex->EA != 1)
			printf("Multiple paths to east texture\n");
		if (tex->NO != 1 || tex->SO != 1
			|| tex->WE != 1 || tex->F != 0 || tex->C != 0)
			printf("Wrong order\n");
		if (tex->map)
			printf("Wrong order: map before texture path");
		free_dbl_array(tmp);
